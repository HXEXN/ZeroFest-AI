...
| 1회 | 1,596 | 6.96 | ±0.43 |
| **6회** | **9,638** | **6.10** | **±0.22** |
| 24회 | 38,528 | 5.85 | ±0.00 |

**정확도는 독립 축제 6회분에서 최고 성능의 5% 이내에 도달한다.** 다만 평균만 보면 안 된다. 1회로 학습하면 어떤 축제를 뽑았느냐에 따라 MAE가 ±0.43까지 흔들리고, 반복 간 편차가 평균의 2% 아래로 내려가는 시점은 **4회**다. 둘 중 큰 값인 6회를 준비 기준으로 삼는다.

축제 횟수를 못 늘릴 때는 규모가 대안이다. 같은 6회라도 `mvp · 12부스 × 4메뉴`은 MAE 6.28, `full · 30부스 × 8메뉴`은 5.89다.

실데이터가 한 번 들어왔을 때는 합성 사전학습이 요구량을 줄인다. 학습 분포보다 수요가 35% 높은 미관측 축제에서, **같은 실데이터 양이면 사전학습을 얹은 쪽이 모든 구간에서 낫다.** 실데이터 25%에 사전학습을 얹은 구성(MAE 8.74)이 실데이터 50% 단독(MAE 9.05)보다 나아, 이 구간에서는 요구량이 약 2분의 1로 줄어든다.
