...
The whole Simulation page reruns every 2 seconds so every visible element
(headers, booth name, metrics, chart, sync diagnostics, actions and messages)
is rebuilt from the latest committed SQLite state.
"""
...
# Full-page refresh: unlike st.fragment, this causes the entire Simulation
# script to rerun, so headers/text/metrics/charts/diagnostics all read the
# latest committed SQLite state together.
AUTO_REFRESH_MS = 2_000
refresh_count = st_autorefresh(
    interval=AUTO_REFRESH_MS,
    limit=None,
    key="simulation-full-page-autorefresh",
)
